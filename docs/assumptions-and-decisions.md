...
| CR-64 | A trusted full-project reset is allowed only before real participants join. | It defaults to dry-run; refuses CI; requires exact repeated project IDs; requires a matching authenticated `.gac-backup` less than 24 hours old plus `RESET PROJECT_ID` before apply; clears the Realtime Database root and canonical anonymous Auth accounts only. Email/Password organizers, all custom claims, and every other persistent Auth account remain untouched. The command changes no Rules, providers, App Check, GitHub, or Firebase project settings and is rehearsed only with synthetic emulator data. |
| CR-65 | Encrypted backup restore automation is limited to a development-only recovery rehearsal. | The command is locked to `games-and-castles-dev`, refuses CI and divergent targets, requires a recent authenticated backup plus `RESTORE games-and-castles-dev`, preserves and verifies persistent Auth accounts, and recreates only missing backed-up anonymous UIDs before restoring the database. Production restore remains a separately reviewed manual recovery decision. |
