...
| CR-60 | Phase 11 win celebrations are derived locally from newly observed authoritative completion events, and participant avatars use a 16-icon Rules allowlist. | Initial run state is baselined so refresh/reconnect does not replay historical wins. Match/session cards keep a crown winner marker, the private view personalizes only by authenticated participant ID, reduced motion omits confetti, and no celebration state or new personal data is persisted. |
