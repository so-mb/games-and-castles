...
Trusted local backup, inspection, cleanup, pre-participant project reset, incident, and credential instructions are centralized in the [Operations runbook](operations-runbook.md). These commands never belong in CI. Remote commands require `GOOGLE_APPLICATION_CREDENTIALS` outside the repository, mode `600`, a matching credential `project_id`, `--confirm-project` equal to `--project`, and the exact root Realtime Database URL through `--database-url`. Emulator mode accepts only `demo-*` IDs and uses no production credential; the full-reset command also requires exact project confirmation in emulator mode and contains its own CI refusal.
